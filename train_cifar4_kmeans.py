...
import dataset.cifar4_kmeans as dataset
...
def main():
    ...
    train_labeled_set, train_unlabeled_set, val_set, test_set = dataset.get_cifar4('./data', args.n_labeled, transform_train=transform_train, transform_val=transform_val)
    ...
    def create_model(ema=False):
        model = models.WideResNet(num_classes=4)
        model = model.cuda()
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch, use_cuda):
    ...
    for batch_idx in range(args.train_iteration):
        ...
        targets_x = torch.zeros(batch_size, 4).scatter_(1, targets_x.view(-1,1).long(), 1)
